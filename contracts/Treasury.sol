...
contract Treasury is Ownable, ITreasury {
    ...
    constructor(address assetsAddress, address auctionAddress) {
    ...
    function checkTrade(uint256 tokenId) external {
        ...
        require(tmp.oldOwner != address(0) || tmp.newOwner != address(0), "Treasury: this trade doesn't exist");
        ...
        require(tmp.paid, "Treasury: buyer has not paid yet");
        if (msg.sender == tmp.oldOwner) {
            (bool success,) = msg.sender.call{value: tmp.price - tmp.time / 100 * FEE}("Your token has been purchased");
    ...
    function pay(uint256 tokenId) external payable {
        PendingTrade memory tmp = _pendingTrades[tokenId];
        require(tmp.oldOwner != address(0) || tmp.newOwner != address(0), "Treasury: this trade doesn't exist");
        require(block.timestamp < tmp.time + DURATION, "Trasury: trade time expired");
    ...
    function addNewPendingTrade(address oldOwner, address newOwner, uint256 tokenId, uint256 time, uint256 price) external {
        ...
        _pendingTrades[tokenId] = PendingTrade(oldOwner, newOwner, uint32(price), uint32(time), false);
